...
def _tokenize(
    ...
    output = dict(
        ...
        response_token_ids_start_idx=response_token_ids_start_idx,
    )
...
def _process_tokens(example: Dict[str, Any], model: "PreTrainedModel" = None, **kwargs) -> Dict:
    ...
    response_token_ids_start_idx = torch.tensor(example["response_token_ids_start_idx"])

    batch = {
        f"{kwargs['prefix']}prompt": prompt,
        f"{kwargs['prefix']}completion": completion,
        f"{kwargs['prefix']}label": example["label"],
        f"{kwargs['prefix']}response_token_ids_start_idx": response_token_ids_start_idx,
    }
...
class DROTrainer(Trainer):
    ...
    def forward(
        ...
        logits, _, value = model(
            batch["completion_input_ids"],
            attention_mask=batch["completion_attention_mask"],
            **model_kwargs,
        )
        value = value.gather(dim=1, index=batch["response_token_ids_start_idx"].unsqueeze(0))
        ...
        # else:
        return (completion_logps, completion_logps, value)
    ...
    def dro_loss(
        ...
        losses = - 0.5 * (label - value - self.beta * logratios).pow(2)
        rewards = self.beta * logratios.detach()
    ...
    def get_batch_loss_metrics(
        ...
        num_chosen = torch.Tensor([len(chosen_idx)]).to(self.accelerator.device)
        num_rejected = torch.Tensor([len(rejected_idx)]).to(self.accelerator.device)

        all_num_chosen = self.accelerator.gather(num_chosen).sum().item()
        all_num_rejected = self.accelerator.gather(num_rejected).sum().item()

        if all_num_chosen > 0:
            metrics["rewards/chosen_sum"] = self.accelerator.gather(rewards[chosen_idx].nansum()).nansum().item()
            metrics["logps/chosen_sum"] = self.accelerator.gather(policy_logps[chosen_idx].nansum()).nansum().item()
            metrics["logits/chosen"] = self.accelerator.gather(policy_logits[chosen_idx].nansum()).nanmean().item()
            metrics["count/chosen"] = all_num_chosen

        if all_num_rejected > 0:
            metrics["rewards/rejected_sum"] = self.accelerator.gather(rewards[rejected_idx].nansum()).nansum().item()
            metrics["logps/rejected_sum"] = self.accelerator.gather(policy_logps[rejected_idx].nansum()).nansum().item()
            metrics["logits/rejected"] = self.accelerator.gather(policy_logits[rejected_idx].nansum()).nanmean().item()
